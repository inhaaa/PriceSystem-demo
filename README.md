...
[운영 성과](#운영-성과) · [화면 미리보기](#화면-미리보기) · [데모 체험](#데모-체험) · [빠른 시작](#빠른-시작) · [구현 포인트](#구현-포인트)
...
**현재 운영 중인 시스템에는 실제 추천 계산 로직과 정밀 분석 기능이 구현되어 있습니다.** 해당 핵심 구현과 내부 판단 기준은 비공개로 유지하며, 이 공개 데모에는 포함하지 않았습니다.

| 구분 | 운영 시스템 | 공개 데모 |
| --- | --- | --- |
| 추천 계산·정밀 분석 | 실제 추천 계산 로직과 정밀 분석 기능 사용 | 해당 로직 제외, 화면 시연용 고정 샘플로 대체 |
| 업무 데이터 | 실제 업무 데이터 사용 | 새로 만든 가상 데이터만 사용 |
| 체험 범위 | 실제 입찰 업무에 사용하는 시스템 | 화면 구성, 업무 흐름, 검색·필터·CRUD 체험 |

데모의 분석 화면은 결과를 확인하는 화면 구성과 조작 흐름을 보여줍니다. **표시되는 추천 값과 분석 결과는 실제 엔진에서 계산하거나 가져온 값이 아니며**, 운영 버전의 분석 수준이나 추천 성능을 나타내지 않습니다.
...
**2026년 7월 서비스를 오픈**하여 실제 입찰 업무에 활용하고 있으며, 운영 시스템 사용 기간에 아래 **낙찰 2건**을 기록했습니다.

| 시점 | 운영 이력 및 성과 |
| --- | --- |
| 2026-07 | 서비스 오픈 및 실제 업무 활용 시작 |
| 2026-07 | 낙찰 1건 발생 |
| 2026-09 | 낙찰 1건 발생 |

위 성과는 실제 운영 시스템의 사용 기간에 발생한 실적입니다. 공개 데모의 가상 데이터나 고정 추천 샘플로 산출한 결과가 아닙니다.
